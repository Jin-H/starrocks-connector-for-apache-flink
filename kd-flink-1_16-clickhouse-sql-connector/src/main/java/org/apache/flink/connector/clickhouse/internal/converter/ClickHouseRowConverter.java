...
public class ClickHouseRowConverter implements Serializable {
            ...
            case VARBINARY:
                return val -> {
                    if (val instanceof UnsignedInteger) {
                        return ((UnsignedInteger) val).longValue();
                    } else if (val instanceof UnsignedLong) {
                        return ((UnsignedLong) val).longValue();
                    } else {
                        return val;
                    }
                };
            case TINYINT:
                return val -> ((UnsignedByte) val).byteValue();
            case SMALLINT:
                return val -> ((UnsignedByte) val).shortValue();
            case DECIMAL:
                final int precision = ((DecimalType) type).getPrecision();
                final int scale = ((DecimalType) type).getScale();
                return val ->
                        DecimalData.fromUnscaledLong(
                                ((UnsignedLong) val).longValue(), precision, scale);
            case DATE:
                return val -> (int) ((LocalDate) val).toEpochDay();
            case TIME_WITHOUT_TIME_ZONE:
                return val -> (int) (((LocalTime) val).toNanoOfDay() / 1_000_000L);
    ...
    interface SerializationConverter extends Serializable {
        ...
        /**
         * <<<<<<< HEAD Convert an internal field to java object and fill into the {@link
