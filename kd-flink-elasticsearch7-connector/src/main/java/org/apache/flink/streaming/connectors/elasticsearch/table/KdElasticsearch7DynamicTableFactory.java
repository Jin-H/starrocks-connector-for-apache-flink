 ...
 */
@Internal
public class KdElasticsearch7DynamicTableFactory
    ...
    public DynamicTableSource createDynamicTableSource(Context context) {
        ...
        KdElasticsearchConfiguration config =
                new KdElasticsearchConfiguration(configuration, context.getClassLoader());
    ...
    private boolean cacheMissingKey(ReadableConfig tableOptions) {
        return tableOptions.get(CACHE_TYPE).equals(LookupCacheType.PARTIAL)
                && tableOptions.get(PARTIAL_CACHE_CACHE_MISSING_KEY);
    }
        ...
        if (!optional.isPresent()) {
            ...
            if (SinkModeType.FIELD.equals(optional.get())) {
                ...
                if (!fieldOption.isPresent()
                        || StringUtils.isNullOrWhitespaceOnly(fieldOption.get())) {
    ...
    private void validateSource(
            KdElasticsearchConfiguration config, Configuration originalConfiguration) {
        config.getHosts(); // validate hosts
        validate(
                config.getIndex().length() >= 1,
                () -> String.format("'%s' must not be empty", INDEX_OPTION.key()));
